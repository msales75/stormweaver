...
abstract class Frame_Reflower {
  ...
  protected function _collapse_margins() {
    ...
    while ( $n && (!in_array($n->get_style()->display, Style::$BLOCK_TYPES) || ($n->get_style()->position=="absolute"))) {
    ...
    if ( $n ) { // && !$n instanceof Page_Frame_Decorator ) 
      ...
      // $str = $this->_frame->_node->nodeName ." #".$this->_frame->get_id();
      // pre_r("Collapsed margin bottom for $this->_frame");

    }

    // Collapse our first child's margin
    $f = $this->_frame->get_first_child();
    while ( $f && (!in_array($f->get_style()->display, Style::$BLOCK_TYPES) || ($f->get_style()->position == "absolute"))) {
      pre_r( "Looking for next sibling... display=".$f->get_style()->display.", position=".$f->get_style()->position );
    ...
    if ( $f ) {
      ...
      // $str = $this->_frame->get_first_child()->_node->nodeName ." #".$f->get_id();
      // pre_r("Collapsed margin top for $f");
    }
